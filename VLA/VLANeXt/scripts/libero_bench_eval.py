...
def eval_libero(cfg, raw_config_dict=None) -> None:
    assert cfg.eval.finetuned_checkpoint is not None, "cfg.eval.finetuned_checkpoint must not be None!"
    ...
    # [新增] 初始化 wandb，设置 run name 为：父目录名_当前存储目录名
    run_name = f"{output_dir.name}_{eval_dir.name}"
    wandb.init(
        project="vlanext",  # 你可以根据需要修改项目名称
        name=run_name,
        config=raw_config_dict  # 将原始配置字典传入 wandb 以便记录
    )
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        for episode_idx in tqdm.tqdm(range(cfg.eval.num_trials_per_task)):
            ...
            # [新增] 在每个 episode 结束后，向 wandb 实时报告当前成功率
            wandb.log({
                "episode/success": int(done),
                "episode/total_episodes_so_far": total_episodes,
                "episode/current_total_success_rate": (total_successes / total_episodes) * 100
            })
        ...
        # [新增] 在每个 task 结束后，记录当前任务专属的 SR
        wandb.log({
            f"task/{task_description}_success_rate": task_sr * 100,
            "eval/current_total_success_rate": total_sr * 100
        })
    ...
    if total_episodes > 0:
        ...
        # [新增] 记录最终的整体成功率
        wandb.log({"eval/final_success_rate": final_success_rate})
...
if __name__ == "__main__":
    ...
    # [新增] 接收 checkpoint 路径
    parser.add_argument("--checkpoint", type=str, default=None, help="Override eval.finetuned_checkpoint")
    
    # [新增] 接收 task_suite_name
    parser.add_argument("--task_suite", type=str, default=None, help="Override eval.task_suite_name (Options: libero_spatial, libero_object, libero_goal, libero_10)")
    ...
    # 确保 config_dict 中有 eval 这一层级
    if "eval" not in config_dict:
        config_dict["eval"] = {}

    # 如果命令行传入了 checkpoint，就在字典里覆盖它
    if args.checkpoint is not None:
        config_dict["eval"]["finetuned_checkpoint"] = args.checkpoint
        print(f"Overriding finetuned_checkpoint with: {args.checkpoint}")

    # 如果命令行传入了 task_suite，就在字典里覆盖它
    if args.task_suite is not None:
        config_dict["eval"]["task_suite_name"] = args.task_suite
        print(f"Overriding task_suite_name with: {args.task_suite}")

    cfg = DictConfig(config_dict)
    eval_libero(cfg, raw_config_dict=config_dict)