...
def eval_libero(cfg, raw_config_dict=None) -> None:  # [修改] 接收原始字典用于 wandb 记录
    assert cfg.eval.finetuned_checkpoint is not None, "cfg.eval.finetuned_checkpoint must not be None!"
    ...
    wandb.init(
        project="vlanext-libero-plus",  # 项目名可以根据需要自定义
        name=run_name,
        config=raw_config_dict
    )
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        for episode_idx in tqdm.tqdm(range(cfg.eval.num_trials_per_task)):
            ...
            # [新增] 初始化该 Episode 结束时的 wandb 日志字典
            wandb_log_dict = {
            ...
            for cat, stats in category_stats.items():
                if stats['total'] > 0:
                    ...
                    print(msg)
                    log_file.write(msg + "\n")
                    # [新增] 收集各种 category 的成功率
                    wandb_log_dict[f"category/{cat}_success_rate"] = sr * 100

            log_file.flush()
            
            # [新增] 向 wandb 发送合并后的 episode 指标
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task_suite", type=str, default=None, help="Override eval.task_suite_name")
    ...
    # [修改] 传递 raw_config_dict 供 wandb 使用
