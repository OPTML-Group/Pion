...
from pion_optim import Muon, DefaultPion
    ...
    def __call__(self, batch):
        ...
        for sample in batch:
            ...
            if self.load_future_image and "future_image" in sample:
                ...
                f_img = torch.fromnumpy(f_img).permute(2, 0, 1).float() / 127.5 - 1.0
...
def train(config):
    ...
    config['project']['name'] = config['project']['name'].replace('TASK_SUITE', task_suite)
    ...
    if global_rank == 0:
        ...
        # === Setup Directory path with optimizer parameters ===
        opt_V = config['train'].get('optimizer_V', 'Muon')
        opt_L = config['train'].get('optimizer_L', 'Muon')
        opt_A = config['train'].get('optimizer_A', 'Muon')
        opt_O = config['train'].get('optimizer_O', 'Muon')
        
        opt_dir_name = f"V_{opt_V}_L_{opt_L}_A_{opt_A}_O_{opt_O}"
        if any("Pion" in o for o in [opt_V, opt_L, opt_A, opt_O]):
            pion_promotion_steps = int(config['train'].get('pion_promotion_steps', 1))
            pion_ns_steps = int(config['train'].get('pion_ns_steps', 5))
            pion_suppression_steps = max(0, pion_ns_steps - pion_promotion_steps)
            opt_dir_name += f"_PionPromo{pion_promotion_steps}_PionSupp{pion_suppression_steps}_PionNs{pion_ns_steps}"

        save_dir = os.path.join(save_dir, opt_dir_name)
        os.makedirs(save_dir, exist_ok=True)

        wandb_name = f"{wandb_name}_{opt_dir_name}"
    ...
    def get_modality(name: str) -> str:
        if "visual" in name: return "V"
        if "language_model" in name: return "L"
        if "action_head" in name: return "A"
        return "O"

    def force_adamw_aux(name: str, p: torch.Tensor) -> bool:
        if p.ndim < 2: return True
        if "final_layer" in name: return True
        if "embed" in name: return True
        return False

    def opt_for_mod(mod: str) -> str:
        if mod == "V": return opt_V
        if mod == "L": return opt_L
        if mod == "A": return opt_A
        return opt_O

    buckets = {
        ("V", "Muon"): [], ("L", "Muon"): [], ("A", "Muon"): [], ("O", "Muon"): [],
        ("V", "DefaultPion"): [], ("L", "DefaultPion"): [], ("A", "DefaultPion"): [], ("O", "DefaultPion"): [],
        ("V", "AdamW_main"): [], ("L", "AdamW_main"): [], ("A", "AdamW_main"): [], ("O", "AdamW_main"): [],
        ("*", "AdamW_aux"): [],
    }
    ...
    for name, p in model.named_parameters():
        if not p.requires_grad:
            continue
        if force_adamw_aux(name, p):
            buckets[("*", "AdamW_aux")].append((name, p))
            continue

        mod = get_modality(name)
        chosen = opt_for_mod(mod)

        if (mod, chosen) in buckets:
            buckets[(mod, chosen)].append((name, p))
        else:
            buckets[(mod, "AdamW_main")].append((name, p))

    optimizers = []
    base_lr = float(config['train']['learning_rate'])
    weight_decay = float(config['train']['weight_decay'])
    lr_ratio = float(config['train'].get('lr_ratio', 0.1))

    for mod in ["V", "L", "A", "O"]:
        named_list = buckets[(mod, "Muon")]
        if named_list:
            optimizers.append(Muon([p for _, p in named_list], lr=base_lr, weight_decay=weight_decay, rank=global_rank, world_size=world_size))

        named_list = buckets[(mod, "DefaultPion")]
        if named_list:
            optimizers.append(DefaultPion(
                [p for _, p in named_list],
                lr=base_lr, weight_decay=weight_decay,
                rank=global_rank, world_size=world_size,
                promotion_steps=pion_promotion_steps,
                ns_steps=pion_ns_steps,
            ))

    adamw_main_named = []
    for mod in ["V", "L", "A", "O"]:
        adamw_main_named += buckets[(mod, "AdamW_main")]
    if adamw_main_named:
        optimizers.append(AdamW([p for _, p in adamw_main_named], lr=base_lr, weight_decay=weight_decay))

    adamw_aux_named = buckets[("*", "AdamW_aux")]
    if adamw_aux_named:
        optimizers.append(AdamW([p for _, p in adamw_aux_named], lr=base_lr * lr_ratio, weight_decay=weight_decay))

    if global_rank == 0:
        print("\n" + "="*80)
        print("Optimizer Parameter Assignment:")
        print("="*80)
        for mod in ["V", "L", "A", "O"]:
            print(f"\n--- Modality: {mod} ---")
            for opt_name in ["Muon", "DefaultPion", "AdamW_main"]:
                named_list = buckets[(mod, opt_name)]
                if named_list:
                    print(f"  [ Optimizer: {opt_name} ]")
                    for name, p in named_list:
                        print(f"    - {name} | size: {list(p.shape)}")

        # Print layers forced to use AdamW_aux
        print("\n--- Forced AdamW_aux (Auxiliary/1D layers) ---")
        if adamw_aux_named:
            print(f"  [ Optimizer: AdamW_aux ]")
            for name, p in adamw_aux_named:
                print(f"    - {name} | size: {list(p.shape)}")
        else:
            print("  None")
    ...
    lr_schedulers = [
        get_scheduler("cosine", optimizer=opt, num_warmup_steps=config['train']['warmup_steps'], num_training_steps=config['data']['max_steps'])
    ...
    if config['train'].get('resume_path'):
        ...
        if os.path.exists(resume_path):
            ...
            opt_states = checkpoint['optimizer_state_dict']
            ...
            if 'scheduler_state_dict' in checkpoint:
                sch_states = checkpoint['scheduler_state_dict']
                if isinstance(sch_states, list):
                    for sch, state in zip(lr_schedulers, sch_states):
                        sch.load_state_dict(state)
                else:
                    lr_schedulers[0].load_state_dict(sch_states)
        ...
        if do_update:
            ...
            if step % config['project']['log_interval'] == 0 and global_rank == 0:
                if config['project'].get('use_wandb', False):
                    wandb.log({
                        ...
                        "train/lr": lr_schedulers[0].get_last_lr()[0],
                        "step": step
                    })
            if step % config['project']['save_interval'] == 0 and global_rank == 0:
                ...
                torch.save({
                    ...
                    'scheduler_state_dict': [sch.state_dict() for sch in lr_schedulers],
                    'config': config
                }, save_path)
...
def _apply_cli_overrides(config: dict, args: argparse.Namespace) -> dict:
    """Apply optional CLI overrides on top of the YAML config (rank 0 prints)."""
    if args.task_suite_name is not None:
        config['data']['task_suite_name'] = args.task_suite_name
    if args.max_steps is not None:
        config['data']['max_steps'] = int(args.max_steps)
    if args.save_interval is not None:
        config['project']['save_interval'] = int(args.save_interval)
    ...
    if args.warmup_steps is not None:
        config['train']['warmup_steps'] = int(args.warmup_steps)

    if args.optimizer_V is not None:
        config['train']['optimizer_V'] = args.optimizer_V
    if args.optimizer_L is not None:
        config['train']['optimizer_L'] = args.optimizer_L
    if args.optimizer_A is not None:
        config['train']['optimizer_A'] = args.optimizer_A
    if args.optimizer_O is not None:
        config['train']['optimizer_O'] = args.optimizer_O

    if args.pion_promotion_steps is not None:
        config['train']['pion_promotion_steps'] = int(args.pion_promotion_steps)
    if args.pion_ns_steps is not None:
        config['train']['pion_ns_steps'] = int(args.pion_ns_steps)

    if args.run_name is not None:
        config['project']['name'] = args.run_name
    if args.output_dir is not None:
        config['project']['output_dir'] = args.output_dir
    if args.wandb_project is not None:
        config['project']['wandb_project'] = args.wandb_project
    if args.wandb_entity is not None:
        config['project']['wandb_entity'] = args.wandb_entity
    if args.use_wandb is not None:
        config['project']['use_wandb'] = bool(int(args.use_wandb))
...
if __name__ == "__main__":
    ...
    # Data / steps
    parser.add_argument("--task_suite_name", type=str, default=None, help="Override data.task_suite_name (e.g. libero_object_no_noops)")
    parser.add_argument("--max_steps", type=int, default=None, help="Override data.max_steps")
    parser.add_argument("--batch_size", type=int, default=None, help="Override data.batch_size (global, summed across GPUs)")
    parser.add_argument("--save_interval", type=int, default=None, help="Override project.save_interval")
    ...
    # Optimizer routing
    parser.add_argument("--optimizer_V", type=str, default=None, choices=[None, "Muon", "DefaultPion", "AdamW"])
    parser.add_argument("--optimizer_L", type=str, default=None, choices=[None, "Muon", "DefaultPion", "AdamW"])
    parser.add_argument("--optimizer_A", type=str, default=None, choices=[None, "Muon", "DefaultPion", "AdamW"])
    parser.add_argument("--optimizer_O", type=str, default=None, choices=[None, "Muon", "DefaultPion", "AdamW"])

    # DefaultPion knobs
    parser.add_argument("--pion_promotion_steps", type=int, default=None)
    parser.add_argument("--pion_ns_steps", type=int, default=None)

    # Project / wandb
    parser.add_argument("--run_name", type=str, default=None, help="Override project.name (TASK_SUITE in name will still be substituted)")
    parser.add_argument("--output_dir", type=str, default=None)
    parser.add_argument("--wandb_project", type=str, default=None)
    parser.add_argument("--wandb_entity", type=str, default=None)
    parser.add_argument("--use_wandb", type=str, default=None, help="0 / 1 to disable / enable wandb")
