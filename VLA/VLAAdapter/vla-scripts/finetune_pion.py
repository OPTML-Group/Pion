...
# Fan
from pion_optim import Muon, LowRankMuon, DefaultPion
...
class FinetuneConfig:
    ...
    # lowrankmuon
    lowrankmuon_rank_k_V: int = 0
    lowrankmuon_rank_k_L: int = 0
    lowrankmuon_rank_k_A: int = 0

    # pion
    pion_promotion_steps: int = 1
    pion_ns_steps: int = 5

    # per-modality optimizer for params with ndim>=2
    # choices: "Muon" / "DefaultPion" / "LowRankMuon" / "AdamW"
    optimizer_V: str = "Muon"
    optimizer_L: str = "Muon"
    optimizer_A: str = "Muon"
    optimizer_O: str = "Muon"   # other / unknown
...
def get_run_id(cfg) -> str:
    ...
    if cfg.run_id_override is not None:
        ...
        run_id = (
            ...
            f"+ratio-{cfg.lr_ratio}"
            f"+wd-{cfg.weight_decay}"
            f"+optimizer_V-{cfg.optimizer_V}"
            f"+optimizer_L-{cfg.optimizer_L}"
            f"+optimizer_A-{cfg.optimizer_A}"
            f"+pion_promotion_steps-{cfg.pion_promotion_steps}"
            f"+pion_ns_steps-{cfg.pion_ns_steps}"
        )
        if cfg.use_fz:
...
def save_training_checkpoint(
    ...
    if cfg.use_lora and cfg.merge_lora_during_training:
        if cfg.use_minivlm:
            ...
            base_vla = AutoModelForVision2Seq.from_config(config, torch_dtype=torch.bfloat16, trust_remote_code=True)  # Create a new model with configuration, the parameters are randomly initialized
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if cfg.use_minivlm:
        ...
        config = AutoConfig.from_pretrained("pretrained_models/configs", trust_remote_code=True)
        vla = AutoModelForVision2Seq.from_config(config, torch_dtype=torch.bfloat16, trust_remote_code=True).to(device_id)  # Create a new model with configuration, the parameters are randomly initialized
    ...
    # ===== 收集需要训练的 (name, param) =====
    trainable_named = [(f"vla.{name}", p) for name, p in vla.named_parameters() if p.requires_grad]

    if getattr(cfg, "use_l1_regression", False):
        trainable_named += [(f"action_head.{name}", p) for name, p in action_head.named_parameters() if p.requires_grad]

    if getattr(cfg, "use_proprio", False):
        trainable_named += [(f"proprio_projector.{name}", p) for name, p in proprio_projector.named_parameters() if p.requires_grad]
    ...
    # ===== Helper: 这些永远走 AdamW-aux（你原来的通用排除规则）=====
    def force_adamw_aux(name: str, p: torch.Tensor) -> bool:
        if p.ndim < 2:
            return True
        if "cls_token" in name or "reg_token" in name:
            return True
        if "embed" in name and "patch_embed" not in name:
            return True
        if "lm_head" in name:
            return True
        if "action_queries" in name:
            return True
        if "action_head" in name and "fc2" in name:
            return True
        return False

    # ===== 读取 per-modality optimizer 配置 =====
    opt_V = str(getattr(cfg, "optimizer_V", getattr(cfg, "optimizer", "AdamW")))
    opt_L = str(getattr(cfg, "optimizer_L", getattr(cfg, "optimizer", "AdamW")))
    opt_A = str(getattr(cfg, "optimizer_A", getattr(cfg, "optimizer", "AdamW")))
    opt_O = str(getattr(cfg, "optimizer_O", getattr(cfg, "optimizer", "AdamW")))
    ...
    # ===== LowRankMuon 超参 =====
    lowrankmuon_rank_k_V = int(getattr(cfg, "lowrankmuon_rank_k_V", 512))
    lowrankmuon_rank_k_L = int(getattr(cfg, "lowrankmuon_rank_k_L", 256))
    lowrankmuon_rank_k_A = int(getattr(cfg, "lowrankmuon_rank_k_A", 128))
    lowrankmuon_rank_k_O = int(getattr(cfg, "lowrankmuon_rank_k_O", lowrankmuon_rank_k_L))

    def rank_k_for_mod(mod: str) -> int:
        if mod == "V": return lowrankmuon_rank_k_V
        if mod == "L": return lowrankmuon_rank_k_L
        if mod == "A": return lowrankmuon_rank_k_A
        return lowrankmuon_rank_k_O

    # ===== 路由：按模态把 ndim>=2 的参数分到各自 optimizer =====
    buckets = {
        ("V", "Muon"): [], ("L", "Muon"): [], ("A", "Muon"): [], ("O", "Muon"): [],
        ("V", "LowRankMuon"): [], ("L", "LowRankMuon"): [], ("A", "LowRankMuon"): [], ("O", "LowRankMuon"): [],
        ("V", "DefaultPion"): [], ("L", "DefaultPion"): [], ("A", "DefaultPion"): [], ("O", "DefaultPion"): [],
        ("V", "AdamW_main"): [], ("L", "AdamW_main"): [], ("A", "AdamW_main"): [], ("O", "AdamW_main"): [],
        ("*", "AdamW_aux"): [],   # 1D / excluded / special
    }
    ...
    for name, p in trainable_named:
        ...
        if chosen == "Muon":
            buckets[(mod, "Muon")].append((name, p))
        elif chosen == "LowRankMuon":
            buckets[(mod, "LowRankMuon")].append((name, p))
        elif chosen == "DefaultPion":
            buckets[(mod, "DefaultPion")].append((name, p))
        elif chosen == "AdamW":
            buckets[(mod, "AdamW_main")].append((name, p))
        else:
            raise ValueError(f"Unknown per-modality optimizer for mod={mod}: {chosen} (name={name})")

    # ===== Debug 打印（仅在 rank0 执行）=====
    if rank == 0:
        print(f"\n{'='*30} Optimizer Routing Plan {'='*30}")
        print(f"# Configured Optimizers: V={opt_V}, L={opt_L}, A={opt_A}, O={opt_O}")
        print(f"# DefaultPion Config: promotion_steps={getattr(cfg, 'pion_promotion_steps', 1)}, ns_steps={getattr(cfg, 'pion_ns_steps', 5)}")

        def _print_group(tag, named_list, optimizer_name, extra_info=""):
            total_elements = sum(p.numel() for _, p in named_list)
            if total_elements == 0:
                return

            # 打印组头信息
            header = f"Group: [{tag}] | Opt: {optimizer_name}"
            if extra_info:
                header += f" | {extra_info}"
            header += f" | Total Elements: {total_elements:,}"
            
            print("-" * 60)
            print(header)
            
            # 打印每一层
            for i, (name, p) in enumerate(named_list):
                print(f"  {name:<60} {str(list(p.shape)):>20}")
            print("")

        # 1. 遍历主要模态的 buckets
        for mod in ["V", "L", "A", "O"]:
            methods = ["Muon", "LowRankMuon", "DefaultPion", "AdamW_main"]
            ...
            for method in methods:
                ...
                named_list = buckets[key]
                if len(named_list) == 0:
                    continue

                extra_info = ""
                if method == "LowRankMuon":
                    k = rank_k_for_mod(mod)
                    extra_info = f"rank={k}"
                elif method == "DefaultPion":
                    extra_info = f"promotion={getattr(cfg, 'pion_promotion_steps', 1)}, ns={getattr(cfg, 'pion_ns_steps', 5)}"
        ...
        _print_group(
            tag="*_AdamW_aux", 
            named_list=buckets[("*", "AdamW_aux")], 
            optimizer_name="AdamW", 
            extra_info="Auxiliary (1D/Embed/Bias)"
        )
    ...
    # 2) LowRankMuon
    for mod in ["V", "L", "A", "O"]:
        named_list = buckets[(mod, "LowRankMuon")]
        if len(named_list) == 0: continue
        optimizers.append(LowRankMuon(
            [p for _, p in named_list],
            lr=cfg.learning_rate,
            weight_decay=cfg.weight_decay,
            rank=rank, world_size=world_size,
            rank_k=rank_k_for_mod(mod),
        ))
    ...
    for mod in ["V", "L", "A", "O"]:
        ...
        optimizers.append(DefaultPion(
            ...
            weight_decay=cfg.weight_decay,
            rank=rank, world_size=world_size,
            promotion_steps=getattr(cfg, "pion_promotion_steps", 1),
            ns_steps=getattr(cfg, "pion_ns_steps", 5)
        ))
    ...
    if len(adamw_main_named) > 0:
        optimizers.append(AdamW(
            [p for _, p in adamw_main_named],
            lr=cfg.learning_rate,
            weight_decay=cfg.weight_decay,
        ))
    ...
    if len(adamw_aux_named) > 0:
        optimizers.append(AdamW(
            [p for _, p in adamw_aux_named],
            lr=cfg.lr_ratio * cfg.learning_rate,
            weight_decay=cfg.weight_decay,
        ))

    if len(optimizers) == 0:
        raise RuntimeError("No optimizer is created. Check requires_grad routing.")

    # ===== Scheduler =====
    schedulers = [
        MultiStepLR(opt, milestones=[cfg.num_steps_before_decay], gamma=0.1)
        for opt in optimizers
    ]
    
    # 记录初始 lr
    original_lr = optimizers[0].param_groups[0]["lr"]
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False, disable=not distributed_state.is_main_process) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if distributed_state.is_main_process and gradient_step_idx % cfg.wandb_log_freq == 0:
                ...
                wandb.log({"VLA Train/Learning Rate": schedulers[0].get_last_lr()[0]}, step=log_step)
