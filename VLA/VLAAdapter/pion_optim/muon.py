"""Pion optimizers (VLA-Adapter vendored copy).

Implements the three base optimizers used by
``vla-scripts/finetune_pion.py``:
  ...
  * ``LowRankMuon``  -- Exact-SVD low-rank truncation variant of Muon.

All three optimizers share the same distributed async ``all_gather``
