#!/usr/bin/env bash
# Train pi0.5 finetune with Pion on all 3 tasks, sequentially.
#
# Usage:
#   bash scripts/run_pion.sh
#
# Routing scheme for 'pion' (Pion):
#   - VL tower (paligemma.*) ndim>=2 non-embed/output  -> Muon
#   - Action tower (gemma_expert.* + action_* / state_proj / *_mlp_*) ndim>=2 -> Pion
#   - everything else (1-D, embed, output head, residual) -> AdamW
#
# Pion uses the high-pass NS iteration with k_p=0, k_s=5 by default
# (configured in src/openpi/training/config.py). The optimizer is dispatched
# via --pytorch-optimizer-name=pion.
#
...
echo
echo "All Pion runs finished: ${TASKS[*]}"
