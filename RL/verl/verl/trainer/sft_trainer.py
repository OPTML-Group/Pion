...
from verl.utils.device import auto_set_device, get_device_id, get_device_name
...
class SFTTrainer:
    ...
    @torch.no_grad()
    def _evaluate_gsm8k_accuracy(self, global_step, tracking, is_logging, epoch):
        """Generate answers for GSM8K test set and compute accuracy after each epoch."""
        try:
            from datasets import load_dataset
        except ImportError:
            if is_logging:
                print("[GSM8K eval] `datasets` library not installed, skipping generation evaluation.")
            return None

        from verl.utils.reward_score.gsm8k import compute_score, extract_solution

        tokenizer = self.model_config.tokenizer
        max_samples = int(self.config.trainer.get("eval_gen_samples", 200))
        gen_batch_size = int(self.config.trainer.get("eval_gen_batch_size", 4))
        ...
        prompts = []
        for ex in ds:
            gt = extract_solution(ex["answer"], method="strict")
            if gt is None:
                continue
        ...
        # Disable FSDP2 resharding to avoid per-token all-gather overhead.
        # Without this, each generated token triggers all-gather + reshard for
        # every layer, making generation ~500x slower than necessary.
        fsdp2_enabled = False
        try:
            from verl.utils.fsdp_utils import FSDPModule, set_reshard_after_forward
            if isinstance(model, FSDPModule):
                fsdp2_enabled = True
                set_reshard_after_forward(model, reshard_after_forward=False, recurse=True)
        ...
        for i in tqdm(
            ...
            desc=f"GSM8K eval (epoch {epoch + 1})",
            disable=not is_logging,
        ):
            batch_prompts = prompts[i : i + gen_batch_size]
            batch_gts = ground_truths[i : i + gen_batch_size]
            ...
            for ids, gt in zip(output_ids, batch_gts):
                response = tokenizer.decode(ids[prompt_len:], skip_special_tokens=True)
                score = compute_score(response, gt, method="flexible")
                correct += score
        ...
        if is_logging:
            tracking.log(
                {"val/gsm8k_accuracy": accuracy, "val/gsm8k_correct": correct, "val/gsm8k_total": total},
                step=global_step,
            )
            epoch_label = "Baseline" if epoch < 0 else f"Epoch {epoch + 1}"
            print(f"[{epoch_label}] GSM8K accuracy: {accuracy:.4f} ({correct}/{total})")

        # Restore FSDP2 resharding for training
        if fsdp2_enabled:
            set_reshard_after_forward(model, reshard_after_forward=True, recurse=True)
            model.reshard()
    ...
    def fit(self):
        ...
        eval_generate = self.config.trainer.get("eval_generate", False)

        # Baseline evaluation before any training
        if eval_generate and global_step == 0:
            if is_logging:
                print("[Baseline] Evaluating pretrained model accuracy before training...")
            self._evaluate_gsm8k_accuracy(global_step, tracking, is_logging, epoch=-1)
        ...
        for epoch in range(start_epoch, self.config.trainer.total_epochs):
            ...
            for step_in_epoch, data in enumerate(
                ...
                if is_last_step:
                    if eval_generate:
                        self._evaluate_gsm8k_accuracy(global_step, tracking, is_logging, epoch)
                    if is_logging:
                        print(f"Total time for train steps: {train_time:.2f}s")
                        print(f"Final validation metrics: {last_valid_metric}")
                    return

            # Epoch boundary: run GSM8K generation evaluation
            if eval_generate:
                self._evaluate_gsm8k_accuracy(global_step, tracking, is_logging, epoch)
